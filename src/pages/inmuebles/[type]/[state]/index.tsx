...
const StatePage: NextPage<Props> = ({ estate, title }) => {
...
export const getServerSideProps: GetServerSideProps = async ({ params }) => {
    ...
    const { type, state } = params as { type: string, state: string };
    const estate: EstateInfo[] = estatesInfo.filter( ( item ) => item.type === type ).filter( ( item ) => item.state === state );
